...
>(
  persist<ThemeStateTypes>(
    (set, get) => ({
      ...
      backgroundColors: themeConfig[0].backgroundColors,
      textColors: themeConfig[0].textColors,
      lockedColors: [],
      setTheme: (newTheme: ThemeType) => {
        set({
          name: newTheme.name,
          backgroundColors: newTheme.backgroundColors,
          textColors: newTheme.textColors,
        });
      },
      ...
      buildCustomTheme: (theme: ThemeType) => {
        const { lockedColors } = get();
        const newBgColors = theme.backgroundColors.map((color, index) => {
          const lockedColor = lockedColors.find((lc) => lc.position === index);
          return lockedColor ? lockedColor.hex : color;
        });
        const newTextColors = theme.textColors.map((color, index) => {
          const lockedColor = lockedColors.find((lc) => lc.position === index);
          return lockedColor ? getTextColor(lockedColor.hex) : color;
        });
        return {
          name: "custom",
...
// const ThemeContext = createContext({} as ThemeStateTypes);
...
//     if (JSON.parse(locLockedColors).length > 0) {
//       if (
